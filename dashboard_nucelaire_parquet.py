...
# Le répertoire « data/ » est créé à côté du script.
# Sur Streamlit Cloud il est éphémère ; en local il persiste.
...
_parquet_lock = threading.Lock()   # protège les lectures/écritures concurrentes
...
def _load_parquet_raw() -> pd.DataFrame:
    ...
    ou un DataFrame vide si le fichier n'existe pas / est illisible.
    Utilisé en interne pour la lecture-modification-écriture (sauvegardes, purges).
    """
    ...
    try:
        ...
        if df.empty:
            return pd.DataFrame()
        # Assurer timezone Europe/Paris sur l'index
...
def jours_cache_dict(start: date, end: date) -> dict[date, tuple[str, int]]:
    ...
    """
    # Cohérence : si le parquet est absent, ignorer les métadonnées JSON
    if not PARQUET_PROD_PATH.exists():
        return {}
    ...
    for j in requested:
        # Avancer jusqu'au premier disponible ≥ j (sans jamais revenir)
        ...
        if ptr < n and available[ptr] == j:
            ...
            # Les jours demandés sont strictement croissants → on avance ptr
            ptr += 1
...
def sauvegarder_batch_en_parquet(resultats_par_jour: dict[date, pd.DataFrame]) -> None:
    ...
    for jour, df_wide in resultats_par_jour.items():
        if df_wide is None or df_wide.empty:
            continue
        # Ne jamais persister aujourd'hui : données incomplètes, toujours re-fetchées en direct
        if jour >= today:
            continue
        idx = df_wide.index
        if idx.tz is None:
    ...
    with _parquet_lock:
        ...
        # Nouvelles données : dédupliquer l'index (garde la dernière valeur)
        df_new = pd.concat(dfs_to_add, axis=0)
        df_new = df_new[~df_new.index.duplicated(keep="last")]

        if not df_existing.empty:
            # Union outer → préserve toutes les colonnes connues
        ...
        # Mise à jour du JSON de métadonnées
        meta = _load_jours_meta_raw()
        meta.update(meta_updates)
        with open(JOURS_META_PATH, "w", encoding="utf-8") as f:
            json.dump(meta, f, ensure_ascii=False, indent=2)

    # Invalider le cache de lecture Streamlit après écriture
...
def charger_depuis_parquet_cache(start: date, end: date) -> pd.DataFrame:
    ...
    après chaque sauvegarde ou purge.
    """
...
def stats_parquet() -> dict:
    """Statistiques rapides sur le cache local (lecture JSON directe, sans cache)."""
    meta = _load_jours_meta_raw()
    if not meta:
...
def purger_periode_parquet(start: date, end: date) -> int:
    ...
    with _parquet_lock:
        ...
        # 2) Supprimer les lignes correspondantes dans le Parquet
        df_prod = _load_parquet_raw()
...
init_data_dir()   # crée data/ si nécessaire
...
# Cible le composant BaseUI (datepicker de Streamlit).
# Les dates sélectionnées (début/fin) s'affichent en rose foncé,
# les jours intermédiaires en rose très pâle.
# Les jours antérieurs à la date de début sont grisés (disabled).
st.markdown("""
<style>
/* Dates de début / fin sélectionnées */
...
/* Jours intermédiaires de la plage */
...
/* Jours non disponibles (hors plage min/max ou avant date début) */
...
with st.sidebar:
    ...
    # Sélecteur de plage natif Streamlit :
    #   • 1er clic  → date de début  (tous les jours antérieurs se grisent)
    #   • 2e clic   → date de fin    (la plage se colore en rose pâle)
    #   • Pendant la sélection, Streamlit renvoie un tuple à 1 élément ;
    #     on attend le 2e clic avant de continuer (st.stop()).
    dates = st.date_input(
    ...
    # Normalisation (cas rare où Streamlit renvoie un date simple)
    if isinstance(dates, date):
        dates = (dates,)

    if len(dates) < 2:
        st.info("📅 Cliquez maintenant sur la date de fin.")
        st.stop()   # Suspendre jusqu'à la sélection complète de la plage
...
def charger_periode(start: date, end: date) -> tuple[pd.DataFrame, int, int]:
    ...
    def _est_cache(j: date) -> bool:
        # Aujourd'hui (et futur) : jamais en cache, toujours fetché en direct
        if j >= now.date():
            return False
    ...
    if jours_a_fetcher:
        # Découpage en blocs de BLOC_JOURS jours (1 requête HTTP par bloc)
        ...
        def _process_resultats(
            ...
            if batch:
                sauvegarder_batch_en_parquet(batch)   # 1 I/O Parquet par bloc
                nb_ok += len(batch)
            for j in fails:
        ...
        # Parallélisme conditionnel (séquentiel si 1 seul bloc)
        if len(blocs) == 1:
        ...
        # Cache Streamlit invalidé dans sauvegarder_batch_en_parquet,
        # mais on force ici au cas où aucun jour n'aurait été sauvegardé.
        charger_depuis_parquet_cache.clear()
...
# Résolution adaptive selon la fenêtre temporelle
if nb_jours > 60:
    freq = "3h"
elif nb_jours > 31:
    freq = "2h"
...
# Downsampling pour la heatmap si période longue
...
# Version downsampleée pour les sparklines si période longue
...
for idx, reacteur in enumerate(reacteurs):
    ...
    # hovertemplate simplifié (pas de customdata)
...
# Hlines à 100 % limitées aux MAX_SHAPES premiers sous-graphiques
shapes_hline = []
